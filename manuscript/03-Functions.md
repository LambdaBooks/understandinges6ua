...
У цьому коді, МВФВ використовується, щоб створити об’єкт з методом `getName()`. Цей метод повертає значення `name`, роблячи `name` приватним значенням об’єкта, що повертається.
