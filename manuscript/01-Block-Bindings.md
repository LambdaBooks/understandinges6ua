...
Так склалось, що одна із найхитріших частин програмування на JavaScript - спосіб, яким оголошуються змінні. У більшості С-подібних мов змінні (або їх *зв'язування*) створюються там, де їх оголошують. Проте в JavaScript це не зовсім так. Місце створення ваших змінних залежить від того, як ви їх оголосили і ECMAScript 6 надає вам простіший спосіб контролювати область видимості. Ця глава ілюструє складності розуміння класичного оголошення змінних за допомогою `var`, вводить блочне зв'язування в ECMAScript 6, а тоді пропонує деякі рекомендації з їх використання.

## оголошення за допомогою ключового слова var та виринання

Коли ми оголошуємо змінну за допомогою ключового слова `var`, не має значення, де саме у коді відбувається оголошення - інтерпретатор мови вважає таку змінну оголошеною на початку функції (або на початку глобальної області видимості, якщо оголошення відбулося поза межами функції). Така поведінка називається *виринання*. Розглянемо її на прикладі наступної функції: 
...
Якщо ви не знайомі з JavaScript, скоріш за все ви очікуєте, що змінна `value` буде створена тільки тоді, коли `condition` буде дорівнювати true. Насправді `value` буде створена незалежно від цього. За лаштунками рушій JavaScript змінює функцію `getValue` наступним чином:
...
Оголошення змінної `value` виринає на початок  функції, а її ініціалізація залишається на тому самому місці (у блоці `if`). Це означає, що змінна `value` також доступна в блоці `else`. Проте, якщо звернутись до неї, отримаємо `undefined`, тому що змінна не була ініціалізована.

Часто новачкам у JavaScript розробці важко звикнути до того як працює оголошення змінних, а нерозуміння цієї неочевидної поведінки може призвести до помилок. З цієї причини ECMAScript 6 вводить блочну область видимості для забезпечення більш гнучкого контролю над життєвим циклом змінних.
