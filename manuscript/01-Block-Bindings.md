...
За синтаксисом оголошення змінних за допомогою `let` не відрізняється від оголошення за допомогою `var`. Ви можете просто замінити `var` на `let`, проте тим самим ви обмежите область видимості змінної блоком, в якому вона оголошена (крім того є ще декілька важливих відмінностей, про які ми поговоримо згодом). Через те, що змінна, оголошена за допомогою `let`, не виринає, гарною звичкою буде оголошувати змінні на самому початку блоку, щоб гарантувати їх доступність в усьому блоці. Наприклад:
...
Поведінка цієї версії функції `getValue` більше схожа на те, що ви очікуєте від інших C-подібних мов програмування. Оскільки змінна `value` оголошена за допомогою `let`, а не `var`, оголошення не виринає на верх визначення функції, і змінна `value` буде знищена після виконання блоку `if`. Якщо ж `condition` буде дорівнювати false, `value` ніколи не буде оголошена або проініціалізована.

### Змінні не перевизначаються

Якщо ідентифікатор змінної вже визначений в поточній області видимості, то оголошення за допомогою `let` із тим самим ідентифікатором викличе помилку. Наведемо приклад:
...
В цьому прикладі `count` визначена двічі: перший раз із `var`, другий - із `let`. Оскільки `let` не перевизначає ідентифікатори, які вже визначені в даній області видимості, друге оголошення викликає помилку. Натомість оголошення змінної з `let`у вкладеному блоці із тим самим ідентиікатором не викличе помилку, що й ілюструє наступний код:  
...
Це оголошення із `let` не викличе помилку, тому що воно створить нову змінну `count` лише у виразі `if`, а не в навколишньому блоці. В середині  блоку `if` нова змінна перекриває глобальну `count`, запобігаючи доступу до неї, допоки виконання не покине блок.
