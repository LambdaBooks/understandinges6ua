...
Одним з найбільш схильних до помилок і заплутаних аспектів JavaScript вже давно є shared everything-підхід до завантаження коду. В той час як інші мови мають такі поняття як пакети, JavaScript відстав і все, що визначається в кожному файлі поділяє єдину глобальну область видимості. Тоді, коли веб-застосунки стали більш складними, а частота використання JavaScript зросла, shared everything-підхід почав виявляти проблеми з конфліктами імен, проблеми безпеки та багато іншого. Однією з цілей ECMAScript 6 було вирішення цієї проблеми і наведення порядку в застосунках JavaScript. Ось тут і з’являються модулі.
...
Незважаючи на те, що є три оголошення `import` в даному модулі, код з `example.js` буде виконуватися тільки один раз. Якщо інші модулі в тому ж самому застосунку повинні були імпортувати зв'язування з `example.js`, ці модулі будуть використовувати один і той же екземпляр модуля, що використовує цей код.
...
Ще до ECMAScript 6, веб-браузери мали кілька способів щоб включати JavaScript у веб-застосунки. Цими опціями завантаження скриптів є:
